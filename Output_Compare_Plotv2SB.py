...
if year == 2018:
    if Prob:
        ...
        if qv_val:
            outputFolder2 = "./Output/Analysis_2018Int_qv_"+str(qvfix)+"_Test/"
        else:
...
for i in range(caseL):
    ...
    if (caseNo <= 622 or caseNo>=630):
        if (caseNo < 1037 or caseNo > 1099) and caseNo < 1800:  #Modify for right cases
    ...
    for j in range(len(nBreakRange)):
        ...
        for ii in range(len(valid_cases1)):
            ...
            if nBreak == nBreak_compare:          
                ...
                multiV = [qvert_compare, 86400/nBreak_compare, abs(slope_coarse), abs(slope_fine), valid_cases1[ii], C_error, F_error, T_error, Temp_error, abs(slope_total), ratio, ratioInt, cum_mass_loss, mass_slope, ratioIntm, massLOST];
