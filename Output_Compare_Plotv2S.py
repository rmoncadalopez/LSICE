...
def mass_process(caseNo, year):
    ...
    return ratio, ratioInt, cum_mass_loss, status, mass_slope, ratiomInt, total_melt_coarse[-1]

#MAIN START ##########################################################################
start_case = int(sys.argv[1])
end_case = int(sys.argv[2])
year = int(sys.argv[3])
Bfix = int(sys.argv[4]) #Use 1500
...
for k in range(len(qvertRange)):
    ...
    for j in range(len(Srange)) :
        ...
        for ii in range(len(valid_cases1)):
            ...
            if Qatm == nBreak_compare:          
                ...
                [ratio, ratioInt, cum_mass_loss, status_good, mass_slope, ratioIntm, massLOST] = mass_process(valid_cases1[ii], year)
    ...
    print(nB_vec)
    print(Cslope_vec)
    plot_image(nB_vec, Cslope_vec, nB_vec, Fslope_vec, 'Satm', 'dC/dt', 'Concentration slope versus nBreak for qvert = ' + str(qvert), 'C vs S_qv_' + str(round(qvert,0)), year, outputFolder2)
...
for k in range(len(Srange)):
    ...
    print(qv_vec)
    print(Cslope_vec)
    plot_image(qv_vec, Cslope_vec, qv_vec, Fslope_vec, 'qvert', 'dC/dt', 'Concentration slope versus qvert for nBreak = ' + str(Qatm), 'C vs qv_nB_' + str(round(Qatm,0)), year, outputFolder2 )
...
x = np.zeros(( len(qvertRange) * len(Srange) ))
...
print("Len A: ", len(qvertRange) * len(Srange)) #CHECK
...
#Save text (results)
dimtx = [len(qvertRange), len(Srange)]
...
plt.ylabel(r'Solar Heat Flux S (W $m^{-2}$)')
# plt.xlabel('Melt Time (days)')
# plt.ylabel('Break Time (days)')
# plt.xlabel('Melt Nondimensional Time')
# plt.ylabel('Break Nondimensional Time')
...
plt.ylabel(r'Solar Heat Flux S (W $m^{-2}$)', fontsize=20) 
# plt.xlabel('Melt Time (days)')
# plt.ylabel('Break Time (days)')
# plt.xlabel('Melt Nondimensional Time')
# plt.ylabel('Break Nondimensional Time')
